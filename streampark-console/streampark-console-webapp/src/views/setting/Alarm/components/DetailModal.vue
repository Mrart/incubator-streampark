...
<script setup lang="ts" name="AlertDetailModal">
  ...
  const dingTalkColumn: DescItem[] = [
    ...
    { label: t('setting.alarm.dingTalkUser'), field: 'contacts' },
    { label: t('setting.alarm.dingtalkIsAtAll'), field: 'isAtAll', render: renderTag },
  ];
  const larkColumn: DescItem[] = [
    { label: t('setting.alarm.larkToken'), field: 'token', span: 2, render: renderTypl },
    {
    ...
    { label: t('setting.alarm.larkIsAtAll'), field: 'isAtAll', render: renderTag },
  ];
  ...
  <BasicModal :show-ok-btn="false" @register="registerModal" class="alert-detail">
    ...
    <template v-if="alertTypeTags.includes('1')">
      ...
      <Description
        ...
        :column="1"
        :data="emailInfo"
        :schema="[{ label: t('setting.alarm.alertEmail'), field: 'contacts' }]"
      />
    </template>
    ...
    <template v-if="alertTypeTags.includes('4')">
      ...
      <Description
        ...
        :column="1"
        :data="weChat"
        :schema="[{ label: t('setting.alarm.weChattoken'), field: 'token', render: renderTypl }]"
      />
    </template>
